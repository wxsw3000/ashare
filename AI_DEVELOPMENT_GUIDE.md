...
#### 规则 3：严格强类型约束与风控开关
- **硬止损 (`stop_loss_pct`)**：填入负数如 `-10.0`（代表 -10% 硬止损）。若填入 `null` 或 `0`，则**完全关闭硬止损**。
- **移动止盈 (`trailing_stop_pct`)**：填入负数如 `-15.0`（代表最高价回撤 15% 止盈）。若填入 `null` 或 `0`，则**完全关闭移动止盈**（完全由策略卖出信号/指标决定何时平仓）。
  > 💡 **实战风控调优建议**：
  > 1. `trailing_stop_pct: null` (关闭)：能完整吃到 50%~100% 的主升浪，但熊市容易回撤坐过山车；
  > 2. `trailing_stop_pct: -5.0` (过窄)：能将最大回撤压至 16% 以内，但容易在 5% 的日常洗盘中被震出局；
  > 3. `trailing_stop_pct: -15.0 ~ -20.0` (宽幅，推荐)：既能过滤日常 5%~10% 洗盘拿大波段，又能在暴涨 100% 见顶回落 15% 时果断锁定 +85% 真实利润；
  > 4. `market.enable_market_trend: true` (大盘择时)：当指数跌破均线时策略 100% 强制空仓持币，彻底避开大盘单边熊市回撤。
- 百分比数值采用小数或百分点数：`roe_min` 填写 `0.05`（代表 5%）；`min_convert_premium` 填写 `-20.0`（代表 -20%）。
...
2. **标的行情绝对隔离 (Zero Pointer Contamination)**：
   全市场多股票行情必须按 `(stock_code, date)` 独立提取与深拷贝。严禁在内存池中共享可变 DataFrame 引用，彻底杜绝某只妖股涨幅被错误克隆给同天其他标的的指针污染问题。
3. **资金账本绝对平衡守恒 (Strict Capital Ledger Conservation)**：
   总资产在任何交易日数学上必须严格满足：
   $$ \text{Total Equity} \equiv \sum_{i=1}^{N} \text{Slot Cash}_i + \sum \text{Market Values} $$
   买入时可用现金必须扣除成本与佣金；卖出时净回款（扣除印花税与佣金）必须全额累加入账，杜绝任何复利膨胀或利润抹平漏洞。
4. **统一回测与实盘执行引擎 (Unified Execution Parity)**：
   回测引擎与模拟实盘持仓底层必须 100% 同构，均基于 `EqualSlotPortfolioStrategy` 插件。严禁编写脱离实盘风控的独立简化版仿真逻辑。
5. **交易手数约束**：
   - 股票买入必须为 **100 股的整数倍** (`floor(avail_budget / price / 1.00025 / 100) * 100`)。
