...
def update_stock_data(conn, code, target_year, target_quarter, update_date, db_buffer, db_buffer_limit):
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT MAX(stat_date) FROM stock_growth_quarterly WHERE code = %s", (code,))
        row = cur.fetchone()
    ...
    while year < target_year or (year == target_year and quarter <= target_quarter):
        ...
        for row in data_list:
            db_row = parse_growth_row(row, update_date)
            if db_row:
...
def main():
    ...
    try:
        ...
        print(f"\n[2] Querying existing growth data from database for {len(all_stocks)} stocks...")
