...
# -*- coding: utf-8 -*-
"""
stock_growth_quarterly 季频成长能力数据更新脚本
从 stock_basic 获取股票列表（type='1'），从 Baostock 拉取成长能力数据
"""
...
GROWTH_FIELDS = "code,pubDate,statDate,YOYEquity,YOYAsset,YOYNI,YOYEPSBasic,YOYPNI"
...
def get_growth_latest_dates(conn, codes):
    """
    批量查询各股票在 growth 表中的最新 stat_date
    ...
    try:
        with conn.cursor() as cur:
            # 检查表是否存在
            cur.execute("SHOW TABLES LIKE 'stock_growth_quarterly'")
            if cur.fetchone() is None:
                print("  [INFO] stock_growth_quarterly table does not exist yet", flush=True)
                for code in codes:
                    result[code] = None
                return result
            
            # 检查表是否为空
            cur.execute("SELECT COUNT(*) FROM stock_growth_quarterly")
            count = cur.fetchone()[0]
            if count == 0:
                print("  [INFO] stock_growth_quarterly is empty, all stocks need full update", flush=True)
                for code in codes:
                    result[code] = None
                return result
    except Exception as e:
    ...
    try:
        ...
        print(f"  [ERROR] Failed to query growth latest dates: {e}", flush=True)
        for code in codes:
            result[code] = None
...
def fetch_growth_data(code, year, quarter, max_retries=3):
...
def build_insert_sql(table_name, fields, batch_size):
    ...
    ON DUPLICATE KEY UPDATE
        ...
        `YOYEquity` = VALUES(`YOYEquity`),
        `YOYAsset` = VALUES(`YOYAsset`),
        `YOYNI` = VALUES(`YOYNI`),
        `YOYEPSBasic` = VALUES(`YOYEPSBasic`),
        `YOYPNI` = VALUES(`YOYPNI`),
...
def flush_db_buffer(conn, batch_data):
    ...
    fields = ['code', 'stat_date', 'pub_date', 'YOYEquity', 'YOYAsset',
              'YOYNI', 'YOYEPSBasic', 'YOYPNI', 'update_date']
    
    sql = build_insert_sql('stock_growth_quarterly', fields, len(batch_data))
...
def parse_growth_row(row, update_date):
    code = row[0]
    ...
    return (
        ...
        stat_date,
        pub_date,
        safe_float(row[3]),  # YOYEquity
        safe_float(row[4]),  # YOYAsset
        safe_float(row[5]),  # YOYNI
        safe_float(row[6]),  # YOYEPSBasic
        safe_float(row[7]),  # YOYPNI
        update_date
    )
...
def main():
    ...
    print("=" * 70)
    print("  [UPDATE] 季频成长能力数据同步 (stock_growth_quarterly)")
    ...
    try:
        ...
        # ====== Step 3: 查询各股票已有的最新数据 ======
        print(f"\n[3] Querying existing growth data from database for {len(all_stocks)} stocks...", flush=True)
        growth_status = get_growth_latest_dates(conn, all_stocks)
        with_data_count = len([v for v in growth_status.values() if v is not None])
        ...
        for code in all_stocks:
            processed += 1
            last_date = growth_status.get(code)
            ...
            while year < target_year or (year == target_year and quarter <= target_quarter):
                ...
                data_list, ok = fetch_growth_data(code, year, quarter)
                ...
                for row in data_list:
                    db_row = parse_growth_row(row, today_str)
                    if db_row:
