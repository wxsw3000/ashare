    ...
    try:
        ...
        for idx, stock in enumerate(all_stocks, 1):
            ...
            if skipped > 0:
                ...
                if has_div:
                    print(f"  {prefix} {code} | 写入 {rows} 条 data | {ipo_date} ~ {target_date} (检测到除权，全量重刷) | 耗时: {stock_elapsed:.3f}s", flush=True)
                else:
                    print(f"  {prefix} {code} | 写入 {rows} 条 data | {start_date} ~ {target_date} | 耗时: {stock_elapsed:.3f}s", flush=True)
