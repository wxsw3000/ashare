...
def get_cash_flow_latest_dates(conn, codes):
    """
    批量查询各股票在 cash_flow 表中的最新 stat_date
...
def parse_cash_flow_row(row, update_date):
    ...
    return (
        ...
        stat_date,
        pub_date,
        safe_float(row[3]),  # CAToAsset
        safe_float(row[4]),  # NCAToAsset
        safe_float(row[5]),  # tangibleAssetToAsset
        safe_float(row[6]),  # ebitToInterest
        safe_float(row[7]),  # CFOToOR
        safe_float(row[8]),  # CFOToNP
        safe_float(row[9]),  # CFOToGr
        update_date
    )
...
def main():
    ...
    try:
        ...
        # ====== Step 3: 查询各股票已有的最新数据 ======
        print(f"\n[3] Querying existing cash_flow data from database for {len(all_stocks)} stocks...", flush=True)
        ...
        for code in all_stocks:
            ...
            while year < target_year or (year == target_year and quarter <= target_quarter):
                ...
                for row in data_list:
                    db_row = parse_cash_flow_row(row, today_str)
                    if db_row:
