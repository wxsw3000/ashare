...
# -*- coding: utf-8 -*-
"""
stock_balance_quarterly 季频偿债能力数据更新脚本
从 stock_basic 获取股票列表（type='1'），从 Baostock 拉取偿债能力数据
"""
...
BALANCE_FIELDS = "code,pubDate,statDate,currentRatio,quickRatio,cashRatio,YOYLiability,liabilityToAsset,assetToEquity"
...
def get_balance_latest_dates(conn, codes):
    """
    批量查询各股票在 balance 表中的最新 stat_date
    ...
    try:
        with conn.cursor() as cur:
            # 检查表是否存在
            cur.execute("SHOW TABLES LIKE 'stock_balance_quarterly'")
            if cur.fetchone() is None:
                print("  [INFO] stock_balance_quarterly table does not exist yet", flush=True)
                for code in codes:
                    result[code] = None
                return result
            
            # 检查表是否为空
            cur.execute("SELECT COUNT(*) FROM stock_balance_quarterly")
            count = cur.fetchone()[0]
            if count == 0:
                print("  [INFO] stock_balance_quarterly is empty, all stocks need full update", flush=True)
                for code in codes:
                    result[code] = None
                return result
    except Exception as e:
    ...
    try:
        ...
        print(f"  [ERROR] Failed to query balance latest dates: {e}", flush=True)
        for code in codes:
            result[code] = None
...
def fetch_balance_data(code, year, quarter, max_retries=3):
...
def build_insert_sql(table_name, fields, batch_size):
    ...
    ON DUPLICATE KEY UPDATE
        ...
        `YOYLiability` = VALUES(`YOYLiability`),
        `liabilityToAsset` = VALUES(`liabilityToAsset`),
        `assetToEquity` = VALUES(`assetToEquity`),
...
def flush_db_buffer(conn, batch_data):
    ...
    fields = ['code', 'stat_date', 'pub_date', 'currentRatio', 'quickRatio',
              'cashRatio', 'YOYLiability', 'liabilityToAsset', 'assetToEquity', 'update_date']
    
    sql = build_insert_sql('stock_balance_quarterly', fields, len(batch_data))
...
def parse_balance_row(row, update_date):
    code = row[0]
    ...
    return (
        ...
        stat_date,
        pub_date,
        safe_float(row[3]),  # currentRatio
        safe_float(row[4]),  # quickRatio
        safe_float(row[5]),  # cashRatio
        safe_float(row[6]),  # YOYLiability
        safe_float(row[7]),  # liabilityToAsset
        safe_float(row[8]),  # assetToEquity
        update_date
    )
...
def main():
    ...
    print("=" * 70)
    print("  [UPDATE] 季频偿债能力数据同步 (stock_balance_quarterly)")
    ...
    try:
        ...
        # ====== Step 3: 查询各股票已有的最新数据 ======
        print(f"\n[3] Querying existing balance data from database for {len(all_stocks)} stocks...", flush=True)
        balance_status = get_balance_latest_dates(conn, all_stocks)
        with_data_count = len([v for v in balance_status.values() if v is not None])
        ...
        for code in all_stocks:
            processed += 1
            last_date = balance_status.get(code)
            ...
            while year < target_year or (year == target_year and quarter <= target_quarter):
                if processed % 50 == 0 and stock_rows == 0:
                    print(f"  [{processed}/{total_stocks}] {code} ({year}Q{quarter}...) ...", end=" ", flush=True)
                
                data_list, ok = fetch_balance_data(code, year, quarter)
                ...
                for row in data_list:
                    db_row = parse_balance_row(row, today_str)
                    if db_row:
            ...
            if stock_rows > 0:
                ...
                if processed % 50 == 0:
                    print(f"[SUCCESS] {stock_rows} 行", flush=True)
