...
def parse_loan_row(row):
    """
    字段顺序: pubDate, loanRate6Month, loanRate6MonthTo1Year, loanRate1YearTo3Year,
    ...
    return (
        pub_date,
        safe_float(row[1]),  # loan_6m
        safe_float(row[2]),  # loan_1y (6个月至1年)
        safe_float(row[3]),  # loan_3y (1年至3年)
        safe_float(row[4]),  # loan_5y (3年至5年)
        safe_float(row[5]),  # loan_above_5y
        safe_float(row[6]),  # mortgage_below_5y
        safe_float(row[7]),  # mortgage_above_5y
    )
...
def main():
    ...
    try:
        ...
        print(f"\n[2] Fetching loan rate data from Baostock ({start_date} ~ {end_date})...", flush=True)
