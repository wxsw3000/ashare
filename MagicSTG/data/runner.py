...
def get_connection_with_retry(max_retries=3, retry_delay=5):
    ...
    for i in range(max_retries):
        try:
            ...
            if i < max_retries - 1:
                print(f"  [DB] 连接数据库失败 (尝试 {i+1}/{max_retries}): {e}，{retry_delay}秒后重试...", flush=True)
                time.sleep(retry_delay)
            else:
                print(f"  [DB] ❌ 连接数据库最终失败: {e}", flush=True)
                raise
...
def get_target_date():
    ...
    if current_time_str < "18:30:00":
        target_dt = bj_now - timedelta(days=1)
    else:
        target_dt = bj_now
...
def run_script_with_progress(script_name, task_date):
    ...
    if not os.path.exists(script_path):
        for sub in ['stock', 'macro', 'index', 'bond']:
            cand = os.path.join(SCRIPT_DIR, sub, script_name)
